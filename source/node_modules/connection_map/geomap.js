class GeoMap {
	...
	constructor(arg) {
		...
		this.param = {
			...
			boundary: {color: "black", stroke: 0.1}
		};
	...
	setDataset(data) {
		...
		this.elements = this.elements.enter()
			...
			.merge(this.elements)
			.attr("d", this.path)
			.attr("fill", function(d, i) { return that.color((2*d.properties.COWCODE + 1) % 20); })
			.attr("stroke", this.param.boundary.color)
			.attr("stroke-width", this.param.boundary.stroke)
	...
	setTooltip() {
		...
		this.elements
			.on("mousemove", function(d) {
				...
				that.tooltip.html(d.properties.CNTRY_NAME)
	...
	setConnection(data) {
		...
		this.elements
			.on("click", function(d) {
				that.connect(d.properties.COWCODE);
			});
	}
	...
	connect(sel_code) {
		...
		var sel_info = this.connect_data.find(function(d) {
			return d.code == sel_code;
		});

		var enemies = (sel_info == undefined)? []: sel_info.enemies;

		this.elements.attr("fill", function(d) {
			var code = d.properties.COWCODE;
			var cond = (enemies.indexOf(code) == -1) && (sel_code != code);
			return that.color((2*code + cond) % 20);
		});
	}
