class GeoMap {
	...
	constructor(arg) {
		...
		this.color_low = "mintcream"
		this.color_high = "mediumseagreen"

		this.color = d3.scaleOrdinal([this.color_low, this.color_high]);
		...
		this.param = {
			...
			elements: {bound: "black", stroke: 0.4, select: "lightseagreen"},
		};
	...
	setProjection() {
		...
		this.back = this.canvas.append("rect")
	...
	setDataset(data) {
		...
		this.elements = this.elements.enter()
			...
			.merge(this.elements)
			.attr("d", this.path)
			.attr("fill", function(d, i) { return that.color(that.color_low); })
	...
	select(prop) {
		...
		if(prop == null) {
			this.selection = null;
			this.elements.attr("fill", function(d, i) { return that.color("lightyellow"); });
			return;
		}
		
		primary = prop.code;
		secondary = prop.enemies;

		var side1 = prop.side1;
		var side2 = prop.side2;

		this.selection = (this.selection != prop)? prop: null;
		...
		this.elements.attr("fill", function(d) {
			...
			else if ((secondary != undefined) && (secondary.includes(code))) {
				fill = that.color(that.color_high);
			}
			else if (((side1 != undefined) && (side1.includes(code))) && ((side2 != undefined) && (side2.includes(code)))){
				fill = "gray";
			}
			else if ((side1 != undefined) && (side1.includes(code))){
				fill = "darkslateblue";
			}
			else if ((side2 != undefined) && (side2.includes(code))){
				fill = "orangered";
			}
			else {
				fill = that.color(that.color_low);
			}
	...
	setClipPath() {
		...
		this.clip.attr("clip-path", "url(#clip-geomap)");
