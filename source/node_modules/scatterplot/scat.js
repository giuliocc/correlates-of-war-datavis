class Scatterplot {
	...
	constructor(arg) {
		...
		this.param = {
			...
			extent: {zoom: [1, 300], translate: [[0, 0], [this.width+this.x, this.height+this.y]]},
			tooltip: {x: 5, y: -20, on: 400, off: 100},
			elements: {color: ["darkgreen", "lightgreen"], radius: [3, 9], margin: {x: 0, y: 0}},
		};
	...
	setDataset(data) {
		...
		this.x_scale.domain([0, d3.max(this.dataset, function(d) { return d.x; })]);
		this.y_scale.domain([0, d3.max(this.dataset, function(d) { return d.y; }) - this.y_scale(10)]);

		this.color_scale.domain(d3.extent(this.dataset, function(d) { return d.class; }));
		this.size_scale.domain(d3.extent(this.dataset, function(d) { return d.class; }));
		...
		this.elements = this.elements.enter()
			...
			.attr("r", function(d) { return that.size_scale(d.class); })
			.attr("fill", function(d) { return that.color_scale(d.class); })
	...
	setZoom() {
		...
		this.zoom = d3.zoom()
			...
			.on("zoom", function() {
				...
				that.elements.attr("r", function(d) { return that.size_scale(d.class)/d3.event.transform.k; });
			});
