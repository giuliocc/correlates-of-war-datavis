class Gantt {
	...
	setDataset(data) {
		...
		this.x_scale.domain([d3.min(this.dataset, function(d) { return d.start_date; }), d3.max(this.dataset, function(d) { return d.end_date; })]);
		this.y_scale.domain(d3.range(d3.max(this.dataset, function(d) { return d.class; })));
		
		this.elements = this.view.selectAll("rect")
			.data(this.dataset);
	...
	setZoom() {
		...
		this.zoom = d3.zoom()
			...
			.on("zoom", function() {
				...
				that.elements
					.attr("y", function(d) { return that.y_scale(d.class); })
					.attr("height", function(d) { return that.y_scale.bandwidth()/transf.k; })
					.attr("rx", that.param.elements.rx/transf.k)
					.attr("ry", that.param.elements.ry/transf.k);
		...
		this.elements
			.on("mousemove", function(d) {
				if (!that.selected.hasOwnProperty(d.war_id)) {
					that.tooltip = new Wordcloud({war_id: d.war_id, x: 0, y: 0, width: that.param.tooltip.width, height: that.param.tooltip.height, container: that.view, id: d.war_id});
