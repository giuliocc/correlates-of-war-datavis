function generateClassesDataset(original_dataset) {
    ...
    var classes = [{"id": 0, "last_end_date": new Date(0,0,0)}]
    var new_dataset = []

    for(i=0; i<spans_dataset.length; i++){
        var war = spans_dataset[i]
        war.class = null
        
        for(j=0; j<classes.length; j++){
            var current_class = classes[j]
            if(war.start_date > current_class.last_end_date){
                war.class = current_class.id
                current_class.last_end_date = war.end_date
